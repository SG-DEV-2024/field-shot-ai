...
class WtChoiceButton<T> extends StatefulWidget {
  const WtChoiceButton({
    Key? key,
  ...
  @override
  State<WtChoiceButton<T>> createState() => _WtChoiceButtonState<T>();
}

class _WtChoiceButtonState<T> extends State<WtChoiceButton<T>> {
  ...
  void initState() {
    ...
    buttonTitleList = widget.buttonList.mapWithIndex((e, index) => getTitleString(e, index)).toList();
  ...
  onChanged(int index) {
    ...
    if (widget.returnAllValueList) {
      return widget.onChanged(checkList, widget.buttonList.toIndexList(checkList), index);
    } else {
  ...
  Widget getButtonWidget(String title, int index) {
    ...
    return checkList.contains(index) ? buildOnButton(index, title) : buildOffButton(index, title);
  }
  ...
  InkWell buildOffButton(int index, String title) {
    return InkWell(
      ...
      child: Container(
        ...
        constraints: const BoxConstraints(minHeight: 34, minWidth: 42),
        ...
        child: Column(
          ...
          children: [
            Text(
              title,
              style: WtTextStyle.labelLarge.gray2,
            ),
          ],
        ),
      ),
    ...
    return InkWell(
      onTap: () {
        if (checkList.length == widget.minCount ||
            (isAllCheck && widget.minCount != null && widget.useMultiSelection == false)) {
          return;
        }
        if (index == -1) {
          // ignore: void_checks
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              title,
              style: WtTextStyle.labelLarge.back,
            ),
          ],
        ),
      ),
  ...
  addAllUpdateList(int index) {
    setState(() {
      checkList = List.generate(widget.buttonList.length, (index) => index).toList();
    });
    checkList.sort();
    return onChanged(index);
  }
