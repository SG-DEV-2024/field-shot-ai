extension WtIterableSC<T> on Iterable<T> {
  ...
  int sumByWithIndex(int Function(T, int) selector) {
    var sum = 0, index = 0;
    var iterator = this.iterator;
    while (iterator.moveNext()) {
      sum += selector(iterator.current, index++);
    }
    return sum;
  }
  ...
  int count([bool Function(T element)? test]) {
    ...
    return map((element) => testFn(element) ? 1 : 0).reduce((value, element) => value + element);
  }
