...
## 6.X 탄산화 화면 — v0.2 디자인 적용 (C-001 / C-002 / C-003)

> v1 탄산화 모듈의 3개 화면을 v0.2 디자인 시스템(규격조사 mockup과 동일 토큰)으로 재정리. **동작 로직·라우트·데이터 모델은 v1 코드 그대로 유지**, 시각 일관성만 통일. 산출물은 [`spec/탄산화/v0.2_design/mockups/`](../탄산화/v0.2_design/mockups/)에 위치.

| 화면 ID | 라우트 | 페이지 / 컨트롤러 | mockup |
|---------|--------|---------------------|--------|
| **C-001** 촬영 가이드 | `/camera` (`surveyType: carbonation`) | [`camera_page.dart`](../../lib/pages/camera/camera_page.dart) / `CameraController2` | [C-001_camera.html](../탄산화/v0.2_design/mockups/C-001_camera.html) |
| **C-002** 사진 보관함 | `/archive` | [`archive_page.dart`](../../lib/pages/archive/archive_page.dart) / `ArchiveController` | [C-002_archive.html](../탄산화/v0.2_design/mockups/C-002_archive.html) |
| **C-003** 데이터 입력 | `/data-input` (`surveyType: carbonation`) | [`data_input_page.dart`](../../lib/pages/data_input/data_input_page.dart) / `DataInputController` | [C-003_data_input.html](../탄산화/v0.2_design/mockups/C-003_data_input.html) |

**핵심 변경 (v1 → v0.2 디자인)**

- 모든 색상 토큰을 v0.2 시스템과 통일 (`#2563EB` / `#f59e0b` / `#22c55e` / `#ef4444` 등)
- **C-001 잠금 상태 시각 강화**: v1은 크로스헤어만 빨강 → v0.2는 가이드(사각형+원) + 안내 버블도 빨강 톤으로 전환해 잠금 인지 향상
- **C-002 카드 썸네일**: v1 사진 풍 → v0.2 캘리퍼스 아이콘(S-001 메인 카드와 동일)으로 통일
- **C-003 키보드 buffer 120px**: S-005/S-008과 동일 정책 적용 (Flutter `resizeToAvoidBottomInset:true` 위에 안전 여유)
- **C-003 단일 저장 액션**: "저장 후 계속 촬영"만 노출 (S-005/S-008 정책과 동일). v1의 이중 버튼 → 단일화는 [data_input_page.dart](../../lib/pages/data_input/data_input_page.dart) 코드 변경으로 완료

**카메라 흐름 (v1 그대로)**

```
S-001 메인 → 탄산화 카드 → C-001 카메라
  └ 셔터 (수평 시) → C-003 데이터 입력
                       └ 저장 → C-001 (연속 측정)
                       └ 다시 촬영 → C-001
  └ 좌하단 썸네일 → C-002 보관함
                       └ 수정 → 모달 → C-002 복귀
...
- `/camera` · `/data-input` · `/archive` 라우트는 **하나** — `surveyType` / `measurementSubtype` 인자로 페이지 내부 분기
- 보관함 페이지는 **통합** — 탄산화·규격조사 레코드 모두 한 리스트에 표시 (S-009가 통합 mockup). C-002는 v1 디자인 재정리 시 명확성을 위해 탄산화만 표시한 변형이지만, 실제 앱은 S-009 패턴 적용 권장
- 자세(기울기) 잠금은 탄산화 카메라(C-001)에서만 적용 — 규격조사 카메라는 미적용
...
| [`v0.2_design/mockups/S-001 ~ S-009.html`](v0.2_design/mockups/) | 규격조사 9개 화면 단독 mockup + 명세 | 개발자 픽셀·코드 값 참조 |
| [`탄산화/v0.2_design/mockups/C-001 ~ C-003.html`](../탄산화/v0.2_design/mockups/) | 탄산화 3개 화면 mockup + 명세 (v0.2 디자인 시스템 적용, 동작은 v1 유지) | 개발자 픽셀·코드 값 참조 |
| [`v0.2_design/mockups/_embed.js`](v0.2_design/mockups/_embed.js) | **프로토타입 전용** 임베드 스크립트 (탄산화·규격조사 공용) | 실제 앱 코드에 포함하지 않음 |
