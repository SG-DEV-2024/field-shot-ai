...
> **결정**: 기존 "배근 간격 조사" 카드는 v2에서 "규격 조사"로 **명칭/대상 변경**한다. 배근 간격은 규격 조사의 한 유형(`member_gap`)이 아니라 별도 항목이었으나, 줄자 기반 측정으로 분류상 동일하므로 흡수한다. 균열 폭 조사는 비전 인식 방식이 달라 별도 카드로 분리한다.
...
enum MeasurementSubtype {
  wallOrMemberWidth('wall_or_member_width', '벽/기둥/부재 폭 측정'),
  memberGap        ('member_gap',           '부재 간 간격 측정'),
...
| 변경 | `_SurveyCard` 2개 → 3개. "배근 간격 조사" → "규격 조사"로 명칭/콜백 변경 + "균열 폭 조사" 비활성 카드 추가 |
...
| 안내 문구 | "줄자 0을 측정 시작면에 맞추고 촬영하세요" | "줄자 양 끝의 눈금이 모두 보이게 촬영하세요" |
...
| subtype | 문구 |
|---------|------|
| `wall_or_member_width` | "줄자 0을 측정 시작면에 맞추고 촬영하세요" |
