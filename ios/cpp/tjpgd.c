...
//	printf("[cq]");
	...
	while (ndata) {	/* Process all tables in the segment */
		if (ndata < 65) return JDR_FMT1_10;	/* Err: table size is unaligned */
		ndata -= 65;
		d = *data++;							/* Get table property */
		if (d & 0xF0) return JDR_FMT1_11;			/* Err: not 8-bit resolution */
		i = d & 3;								/* Get table ID */
...
//	printf("[ch]");
	...
	while (ndata) {	/* Process all tables in the segment */
		if (ndata < 17) return JDR_FMT1_12;	/* Err: wrong data size */
		ndata -= 17;
		d = *data++;						/* Get table number and class */
		if (d & 0xEE) return JDR_FMT1_13;		/* Err: invalid class/number */
		...
		if (ndata < np) return JDR_FMT1_14;	/* Err: wrong data size */
		ndata -= np;
		...
		for (i = 0; i < np; i++) {			/* Load decoded data corresponds to each code word */
			d = *data++;
			if (!cls && d > 11) return JDR_FMT1_15;
			pd[i] = d;
		}
#if JD_FASTDECODE == 2
...
//				if (*dp != 0) return 0 - (int)JDR_FMT1_16;	/* Err: unexpected flag is detected (may be collapted data) */
				if (*dp != 0) {
//					printf("huffext(%d)n",cls);
					return 0 - (int)JDR_FMT1_16;	/* Err: unexpected flag is detected (may be collapted data) */
				}
	...
	return 0 - (int)JDR_FMT1_17;	/* Err: code not found (may be collapted data) */
}
...
//	printf("b");
	...
	do {
		if (!mbit) {			/* Next byte? */
			...
			if (flg) {			/* In flag sequence? */
				flg = 0;		/* Exit flag sequence */
				if (*dp != 0) return 0 - (int)JDR_FMT1_18;	/* Err: unexpected flag is detected (may be collapted data) */
				*dp = 0xFF;		/* The flag is a data 0xFF */
			} else {
				if (*dp == 0xFF) {		/* Is start of flag sequence? */
...
//	printf("\nenter %s() ", __func__);
	...
	if ((d & 0xFFD8) != 0xFFD0 || (d & 7) != (rstn & 7)) {
		return JDR_FMT1_19;	/* Err: expected RSTn marker is not detected (may be collapted data) */
	}
	...
	if ((marker & 0xFFD8) != 0xFFD0 || (marker & 7) != (rstn & 7)) {
		return JDR_FMT1_20;	/* Err: expected RSTn marker was not detected (may be collapted data) */
	}
	...
	for (blk = 0; blk < nby + 2; blk++) {	/* Get nby Y blocks and two C blocks */
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
			...
			do {
				...
				if (z >= 64) return JDR_FMT1_11;		/* Too long zero run */
...
)
{
	...
	for (;;) {				/* Parse JPEG segments */
		...
		if (len <= 2 || (marker >> 8) != 0xFF) return JDR_FMT1_22;
		...
		case 0xC0:	/* SOF0 (baseline JPEG) */
			...
			if (jd->ncomp != 3 && jd->ncomp != 1) return JDR_FMT3_30;	/* Err: Supports only Grayscale and Y/Cb/Cr */
			...
			for (i = 0; i < jd->ncomp; i++) {
				...
				if (i == 0) {	/* Y component */
					if (b != 0x11 && b != 0x22 && b != 0x21) {	/* Check sampling factor */
						return JDR_FMT3_31;					/* Err: Supports only 4:4:4, 4:2:0 or 4:2:2 */
					}
					jd->msx = b >> 4; jd->msy = b & 15;		/* Size of MCU [blocks] */
				} else {		/* Cb/Cr component */
					if (b != 0x11) return JDR_FMT3_32;			/* Err: Sampling factor of Cb/Cr must be 1 */
				}
				jd->qtid[i] = seg[8 + 3 * i];				/* Get dequantizer table ID for this component */
				if (jd->qtid[i] > 3) return JDR_FMT3_33;		/* Err: Invalid ID */
			}
			break;
		...
		case 0xDA:	/* SOS - Start of Scan */
			...
			if (!jd->width || !jd->height) return JDR_FMT1_13;	/* Err: Invalid image size */
			if (seg[0] != jd->ncomp) return JDR_FMT3_34;		/* Err: Wrong color components */

			/* Check if all tables corresponding to each components have been loaded */
			for (i = 0; i < jd->ncomp; i++) {
				b = seg[2 + 2 * i];	/* Get huffman table ID */
				if (b != 0x00 && b != 0x11)	return JDR_FMT3_35;	/* Err: Different table number for DC/AC element */
				n = i ? 1 : 0;							/* Component class */
				if (!jd->huffbits[n][0] || !jd->huffbits[n][1]) {	/* Check huffman table for this component */
					return JDR_FMT1_24;					/* Err: Nnot loaded */
				}
				if (!jd->qttbl[jd->qtid[i]]) {			/* Check dequantizer table for this component */
					return JDR_FMT1_25;					/* Err: Not loaded */
				}
			}

			/* Allocate working buffer for MCU and pixel output */
			n = jd->msy * jd->msx;						/* Number of Y blocks in the MCU */
			if (!n) return JDR_FMT1_26;					/* Err: SOF0 has not been loaded */
		...
		case 0xCF:	/* SOF15 */
		case 0xD9:	/* EOI */
			return JDR_FMT3_36;	/* Unsuppoted JPEG standard (may be progressive JPEG) */
